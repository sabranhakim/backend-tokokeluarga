...
new class extends Component {
    ...
    {
        if (! Gate::allows('create retur_pembelian') && ! auth()->user()->hasAnyRole(['admin', 'staff'])) {
            session()->flash('error', 'Anda tidak memiliki hak akses untuk menambah retur pembelian.');
    ...
    public function save(ReturPembelianService $service)
    {
        if (! Gate::allows('create retur_pembelian') && ! auth()->user()->hasAnyRole(['admin', 'staff'])) {
        ...
        try {
            ...
            session()->flash('success', 'Retur pembelian berhasil disimpan dan stok telah diperbarui.');
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-1 space-y-6">
                <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
                    <h3 class="text-lg font-bold text-slate-800 mb-6 border-b border-slate-50 pb-2">Informasi Retur Pembelian</h3>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-1">Supplier (Opsional)</label>
        ...
        <div class="bg-white rounded-2xl shadow-2xl border border-slate-100 max-w-2xl w-full overflow-hidden animate-in fade-in zoom-in duration-200">
            <div class="px-6 py-4 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
                <h3 class="text-lg font-black text-slate-800 flex items-center">
                    <svg class="w-6 h-6 text-blue-600 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 15v-1a4 4 0 00-4-4H8m0 0l3 3m-3-3l3-3m9 14V5a2 2 0 00-2-2H6a2 2 0 00-2 2v16l4-2 4 2 4-2 4 2z"/></svg>
                    Konfirmasi & Ringkasan Retur Pembelian
                </h3>
            ...
            <div class="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-sm font-medium text-slate-900 mt-1">{{ $suppliers->firstWhere('id', $supplier_id)?->nama_supplier ?? '-' }}</p>
                    </div>
                    @endif
                </div>
                ...
                <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start">
                    ...
                    <p class="text-sm font-medium text-amber-700">Stok barang akan langsung berkurang setelah disimpan.</p>
                </div>
            </div>
