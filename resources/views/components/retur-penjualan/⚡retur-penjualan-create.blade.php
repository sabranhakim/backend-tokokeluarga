...
new class extends Component {
    ...
    {
        if (! Gate::allows('create retur_penjualan') && ! auth()->user()->hasAnyRole(['admin', 'staff'])) {
            session()->flash('error', 'Anda tidak memiliki hak akses untuk menambah retur penjualan.');
    ...
    public function save(ReturPenjualanService $service)
    {
        if (! Gate::allows('create retur_penjualan') && ! auth()->user()->hasAnyRole(['admin', 'staff'])) {
        ...
        try {
            ...
            session()->flash('success', 'Retur penjualan berhasil disimpan dan stok telah diperbarui.');
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-1 space-y-6">
                <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
                    <h3 class="text-lg font-bold text-slate-800 mb-6 border-b border-slate-50 pb-2">Informasi Retur Penjualan</h3>

                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-1">Tanggal Retur</label>
                            <div class="relative group"
                                 wire:ignore
                                 x-data="{
                                    value: @entangle('tgl_retur'),
                                    instance: null,
                                    init() {
                                ...
                                <input x-ref="input"
                                       type="text"
                                       readonly
                                       class="w-full pl-10 pr-4 py-2 rounded-lg border border-slate-200 outline-none focus:ring-2 focus:ring-blue-500 transition-all bg-white cursor-pointer @error('tgl_retur') border-red-500 @enderror"
                                       placeholder="Pilih Tanggal">
                            </div>
                            @error('tgl_retur') <span class="text-red-500 text-xs font-medium">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-1">Nama Pelanggan (Opsional)</label>
                            <input wire:model="nama_pelanggan" type="text" class="w-full px-4 py-2 rounded-lg border border-slate-200 outline-none focus:ring-2 focus:ring-blue-500 transition-all" placeholder="Nama pelanggan yang mengembalikan barang...">
                            @error('nama_pelanggan') <span class="text-red-500 text-xs font-medium">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-1">Keterangan (Opsional)</label>
                            <textarea wire:model="keterangan" rows="3" class="w-full px-4 py-2 rounded-lg border border-slate-200 outline-none focus:ring-2 focus:ring-blue-500 transition-all resize-none" placeholder="Catatan alasan retur..."></textarea>
                            ...
                            <div wire:key="item-{{ $index }}" class="bg-slate-50/50 p-4 rounded-xl border border-slate-100 relative group transition-all hover:bg-slate-50">
                                <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                                    ...
                                    <div class="md:col-span-2">
                                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Stok Saat Ini</label>
                                        @php
                                            $stokTersedia = 0;
                                            if ($item['barang_id']) {
                                        ...
                                        @endphp
                                        <div class="px-4 py-2 rounded-lg border border-slate-200 bg-slate-100 text-slate-700 text-sm font-bold">{{ $stokTersedia }}</div>
                                    </div>
        ...
        <div class="bg-white rounded-2xl shadow-2xl border border-slate-100 max-w-2xl w-full overflow-hidden animate-in fade-in zoom-in duration-200">
            <div class="px-6 py-4 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
                <h3 class="text-lg font-black text-slate-800 flex items-center">
                    <svg class="w-6 h-6 text-blue-600 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h2m4 0h4M9 21h6a2 2 0 002-2V5a2 2 0 00-2-2H9a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>
                    Konfirmasi & Ringkasan Retur Penjualan
                </h3>
            ...
            <div class="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="text-sm font-medium text-slate-900 mt-1">{{ $tgl_retur ? date('d F Y', strtotime($tgl_retur)) : '-' }}</p>
                    </div>
                    @if($nama_pelanggan)
                    <div>
                        <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Pelanggan</p>
                        <p class="text-sm font-medium text-slate-900 mt-1">{{ $nama_pelanggan }}</p>
                    </div>
                    @endif
                </div>
                ...
                <div>
                    ...
                    <p class="text-sm font-medium text-slate-700 mt-1">{{ $keterangan }}</p>
                </div>
                @endif
                            ...
                            @foreach($items as $item)
                                @if($item['barang_id'])
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-4 py-2.5 text-right font-bold text-slate-900">{{ $item['jumlah'] }} pcs</td>
                                    </tr>
                                @endif
                            @endforeach
                        </tbody>
                ...
                <div class="bg-green-50 border border-green-200 rounded-xl p-4 flex items-start">
                    <svg class="w-5 h-5 text-green-600 mr-2 mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                    <p class="text-sm font-medium text-green-700">Stok barang akan langsung bertambah setelah disimpan.</p>
                </div>
            </div>
